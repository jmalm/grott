...
class DataMessage(Message):
    ...
    def deserialize(self, data: bytes) -> bytes:
        ...
        self.datalogger_id = rest[0:10].decode('ascii')
        return rest[10 + self.buffer_length:]

    @property
    def body(self) -> bytes:
        return (super().body +
                self.datalogger_id.encode('ascii') +
...
class Announcement(DataMessage):
    ...
    def deserialize(self, data: bytes) -> bytes:
        ...
        self.inverter_id = rest[:10].decode('ascii')
        return rest[10:]

    @property
    def body(self):
        return (super().body +
                self.inverter_id.encode('ascii'))
...
def deserialize(data: bytes, message_classes: list[type(Message)]) -> Message:
